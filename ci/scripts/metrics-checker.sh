...
checkCoreApplicationJAVA(){
    echo "implement metrics"
#in the future it has to be uncommented. we have commented out this checking function as it is a poc and the restemplate implementation is in a different class.
#potential fixes: update this checking function to accept different scenario, impelement restemplate definition in core application
...
#
#   find ./src/main/java -regex 'RestTemplateConfiguration.java' -exec grep -o 'public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder)' {} \; > .bob/var.scrapedLine || true;
#    find ./src/main/java -regex 'RestTemplateConfiguration.java' -exec grep -o 'return restTemplate' {} \; >> .bob/var.scrapedLine || true;
#
#    if cmp -s .bob/var.compareToLine .bob/var.scrapedLine; then
